...
def model_features(input_size, params, quant_firstlayer=False):
    if int(params['dense2']) != 0:
        model = keras.Sequential([
            ...
            tf.keras.layers.Dense(int(params['dense1'])),
...
def parse_args():
    ...
    argparser.add_argument('--param_path', type=str, help="path of params", 
                            default=None)
    argparser.add_argument('--param_path50', type=str, help="path of params", 
                            default=None)
    ...
    argparser.add_argument('--enable_nni', type=bool, help="is started by nni", 
                            default=False, choices=[True, False])
    argparser.add_argument('--cluster_param_path', type=str, help="cluster_weight params", 
                            default=None)
    
    args = argparser.parse_args()
    return args

def run_once(count, args, nni_param=None):
    # Hyperparameters
    LR = args.lr
    EPOCH = args.epoch
    MODEL = args.model
    ...
    print("Start Weight Clustering Training")

    if args.enable_nni:
        nni_param = nni.get_next_parameter()
    elif nni_param is None:
        nni_param = {
            "centroid": "Linear",
            "num_cluster": 256,
            "epoch": 2,
    ...
    centroid_map = {
        ...
        "Density": CentroidInitialization.DENSITY_BASED,
        "KPP": CentroidInitialization.KMEANS_PLUS_PLUS
    }

    cluster_weights = tfmot.clustering.keras.cluster_weights
    if nni_param["centroid"] == "KPP":
        nni_param["num_cluster"] = min(12, (int(nni_param["num_cluster"]) // 24) + 2)
    cluster_param = {
        'number_of_clusters': int(nni_param["num_cluster"]),
        'cluster_centroids_init': centroid_map[nni_param["centroid"]]
    }

    my_model = cluster_weights(my_model, **cluster_param)
    
    my_model.compile(optimizer=Adam(learning_rate=LR),
              loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
              metrics=['accuracy'])
    my_model.summary()
    EPOCH = int(nni_param["epoch"])

    cluster_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
        filepath=checkpoint_filepath[:-2] + "cluster/",
        save_weights_only=True,
        monitor='val_accuracy',
        mode='max',
        save_best_only=True
    )
    callbacks = [cluster_checkpoint_callback]
    if int(nni_param["use_swa"]) == 1:
        swa = SWA(start_epoch=start_epoch, 
            lr_schedule='cyclic', 
            swa_lr=0.0001,
            swa_lr2=0.0005,
    ...
    if not args.enable_nni:
        model_save_name = nni_param["centroid"] + "_" + str(nni_param["num_cluster"]) + "_" + str(nni_param["use_swa"]) + "_" + str(nni_param["epoch"])
        my_model.save(f'./train_ckpt/{result_dir}/{count}/' + model_save_name + '_unstrip.h5')
        my_model.save_weights(f'./train_ckpt/{result_dir}/{count}/' + model_save_name + '_unstrip_weights.h5')
    my_model = tfmot.clustering.keras.strip_clustering(my_model)
    print("Final model:")
    my_model.summary()

    if not args.enable_nni:
        save_tf(f'./train_ckpt/{result_dir}/{count}/' + model_save_name + '_None.tflite', my_model, None, x_aug)
        save_tf(f'./train_ckpt/{result_dir}/{count}/' + model_save_name + '_FLOAT16.tflite', my_model, "FLOAT16", x_aug)
    ...
    def get_metric(score_mode):
        ...
        avgFB, G_score, detection_score = stats_report(mylists, None)
    ...
    def get_score(score_mode="all"):
        if score_mode == "all":
            ...
            test_scores["default"] = 0.67*test_scores["testScore"] + 0.33*train_score["trainScore"]
...
def run_and_save(i, args, params, c_params=None):
    args.model_params = params
    scores, _ = run_once(i, args, c_params)
    return scores

def run_group(i, args, params, times, c_params=None):
    metrics = []
    for _ in range(times):
        scores = run_and_save(i, args, params, c_params)
...
if __name__ == '__main__':
    ...
    # train model with below hyper-params
        ...
        print(data)
    elif args.param_path50 is not None:
        result_dir = "cluster"
        for i in range(0, 50):
            times = 1
            best_FB_dict = {}
            import json
            with open(args.param_path50) as file:
                PARAMS = json.load(file)
                PARAMS = [PARAMS[0]]
            with open(args.cluster_param_path) as file:
                cluster_params = json.load(file)
            with multiprocessing.Pool(len(cluster_params)) as pool:
                results = []

                for c_params in cluster_params:
                    result = pool.apply_async(run_group, (i, args, PARAMS[0], times, c_params))
                    results.append(result)

                results = {
                    (
                        str(cluster_params[idx]["centroid"]),
                        str(cluster_params[idx]["num_cluster"]),
                        str(cluster_params[idx]["use_swa"]),
                        str(cluster_params[idx]["epoch"]), 
                        str(cluster_params[idx]["score"])
                    ): result.get()[2] for idx, result in enumerate(results)
                }

            with open(f"./train_result/{result_dir}/summary{i}.csv", 'wb') as f:
                _, score_keys = list(results.items())[-1]
                score_keys = score_keys[0].keys()
                csv_header = "centroid,num_cluster,use_swa,epoch,o_score," + ",".join(score_keys) + "\n"
                f.write(csv_header.encode('utf-8'))
                for idx, (denses, metrics) in enumerate(results.items()):
                    for scores in metrics:
                        csv_values = f"{denses[0]},{denses[1]},{denses[2]},{denses[3]},{denses[4]}"
                        for key in score_keys:
                            csv_values = csv_values + f",{scores[key]}"
                        f.write(f"{csv_values}\n".encode('utf-8'))
            print(results)
