...
result_dir = "5featuresV3adjust_simple"
...
def feature_extract(x: np.ndarray, nni_params, verbose=True):
    ...
    from decision_b2 import countPeaksNN, count_unpeak
    ensemble = 5
    ...
    for idx, sample in enumerate(x):
        ...
        for i in range(len(thresholds)):
            ...
            numPeaks, peaksIdx, cur_gap = countPeaksNN(sample,factor_, std)
            cur_gaps.append(cur_gap)
            numUnpeaks, unpeaksIdx, cur_gap = count_unpeak(sample, factor_, std)
            cur_gaps.append(cur_gap)
    ...
    return features_list, np.mean(cur_gaps)
...
def run_once(count, args):
    ...
    x_aug, avg_gap_train = feature_extract(x_aug, params)
    x_test, avg_gap_test = feature_extract(x_test, params)
    LR = params['lr']
    ...
    print('acc', score[1])
    save_tf(f'./train_ckpt/{result_dir}/{count}/' + save_name + '.tflite', my_model)
    ...
    def get_metric(score_mode):
        ...
        for subject in subjects:
            ...
            test_samples = test_iterator.get_next()
            x_test, y_test = test_samples[...,0:-1], test_samples[...,-1]
            x_test, _ = feature_extract(x_test.numpy(), params)
            x_test = np.expand_dims(x_test, axis=2)
    ...
    def get_score(avg_gap_train, avg_gap_test, score_mode="all"):
        if score_mode == "all":
            test_scores = get_score(avg_gap_train, avg_gap_test, "test")
            train_score = get_score(avg_gap_train, avg_gap_test, "train")
            test_scores.update(train_score)
            test_scores["avg_gap_train"] = avg_gap_train
            test_scores["avg_gap_test"] = avg_gap_test
            return test_scores
        else:
    ...
    scores = get_score(avg_gap_train, avg_gap_test)
...
def process_subject_for_test_model(subject, args, mode, SIZE, weight_path):
    ...
    if str(weight_path).count("model_features") != 0:
        x_test, _ = feature_extract(x_test, params, verbose=False)
