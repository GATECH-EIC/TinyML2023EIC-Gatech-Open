...
def feature_extract(x: np.ndarray, nni_params, enable_normalization=False, scaler=None, enable_fgs=False, selected_indices=None, y=None, verbose=True):
    ...
    n_features = int(nni_params.get("n_features", 20))
    ...
    if enable_fgs:
        if selected_indices is None:
            fgs = FeatureGradientSelector(order=12, n_features=n_features)
            fgs.fit(features_list, y)
            selected_indices = fgs.get_selected_features()
        new_data = []
        for row in features_list:
            selected_columns = [row[i] for i in selected_indices]
            new_data.append(selected_columns)
        features_list = np.array(new_data).astype("float32")

    features_list = features_list.reshape(features_list.shape[0], features_list.shape[1], 1)
    return features_list, scaler, selected_indices
...
def parse_args():
    ...
    argparser.add_argument('--enable_normalization', type=str, help="enable normalization", 
                            default=False)
    argparser.add_argument('--enable_fgs', type=str, help="enable FGS Feature Selector", 
                            default=False)
...
def run_once(count, args):
    ...
    if MODEL == "best":
        ...
        args.enable_normalization = bool(int(params.get("enable_fgs", 0)))
        x_aug, scaler, selected_indices = feature_extract(x_aug, params, args.enable_normalization, None, args.enable_fgs, None, y_aug)
        x_test, scaler, selected_indices = feature_extract(x_test, params, args.enable_normalization, scaler, args.enable_fgs, selected_indices, y_aug)
        LR = params['lr']
    ...
    def get_metric(score_mode, scaler, selected_indices):
        ...
        for subject in subjects:
            ...
            if MODEL == "model_resnet152":
                ...
                x_test, scaler, selected_indices = feature_extract(x_test.numpy(), params, args.enable_normalization, scaler, args.enable_fgs, selected_indices, None)
            # x_test = np.expand_dims(x_test, axis=2)
    ...
    def get_score(score_mode="all", scaler=None, selected_indices=None):
        if score_mode == "all":
            test_scores = get_score("test", scaler, selected_indices)
            train_score = get_score("train", scaler, selected_indices)
            test_scores.update(train_score)
            test_scores["default"] = train_score['trainScore']
            return test_scores
        else:
            avgFB, G_score, detection_score = get_metric(score_mode, scaler, selected_indices)
            return {
    ...
    if not args.enable_normalization:
        scaler = None
    scores = get_score(scaler=scaler, selected_indices=selected_indices)
    if args.enable_fgs:
        scores["features_length"] = len(selected_indices)
...
def process_subject_for_test_model(subject, args, mode, SIZE, weight_path):
    ...
    if str(weight_path).count("model_features") != 0:
        x_test = feature_extract(x_test, params, args.enable_normalization, None,args.enable_fgs, None, y_test, verbose=False)
