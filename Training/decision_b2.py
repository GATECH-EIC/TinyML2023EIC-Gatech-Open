...
def countPeaksNN(chunk,factor, std, detect_gap=20, inp_len=1250):
    ...
    first_part = 300
    scale_factor = 0.7
    while idx < first_part:
        if chunk[idx] > peak_threshold:
            peak_sampled_idx.append(idx)
            idx += detect_gap
        else:
            idx += 1
    if len(peak_sampled_idx) >= 2:
        cur_detect_gap = int(min(np.diff(peak_sampled_idx))*scale_factor)
        cur_detect_gap = max(detect_gap, cur_detect_gap)
    else:
        cur_detect_gap = detect_gap
    while idx < inp_len:
        if chunk[idx] > peak_threshold:
            peak_sampled_idx.append(idx)
            if len(peak_sampled_idx) >= 2:
                cur_detect_gap = int(min(cur_detect_gap, (peak_sampled_idx[-1] - peak_sampled_idx[-2])*scale_factor))
                cur_detect_gap = max(cur_detect_gap, detect_gap)
            else:
                cur_detect_gap = detect_gap
            idx += cur_detect_gap
        else:
    ...
    # truncation works only when three or more peaks are sampled
    if len(peak_sampled_idx) < 3:
        return len(peak_sampled_idx), peak_sampled_idx, cur_detect_gap
    # baseline method
    og_ans = len(peak_sampled_idx)
    # truncation method
    peak_diff = np.diff(np.asarray(peak_sampled_idx))
    ...
    return inter_ans, peak_sampled_idx, cur_detect_gap
