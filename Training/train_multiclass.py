...
class DataGenerator(tf.keras.utils.Sequence):
  ...
  def __getitem__(self, idx):
    ...
    # enable multi-class
    label = LABEL_MAP[self.names_list[idx].split(' ')[0].split("-")[1]]
...
def run_once(count, args):
    ...
    print('acc', score[1])
    save_tf(f'./train_ckpt/{result_dir}/{count}/' + save_name + '.tflite', my_model, args.quant, x_aug)
    ...
    def get_metric(score_mode):
        ...
        for subject in subjects:
            ...
            #enable multi-class
            pred = is_positive(pred)
            y_test = is_positive(y_test.numpy())
