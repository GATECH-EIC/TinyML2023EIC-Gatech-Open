...
POSITIVE_SET.add(4)
POSITIVE_SET.add(5)
def is_positive(labels:list):
    converted = []
    for label in labels:
        if type(label) is np.ndarray:
            label = label[0]
        converted.append(1 if label in POSITIVE_SET else 0)
    return converted
...
def feature_extract(x: np.ndarray, path, verbose=True):
    verbose = False
    ...
    for idx, sample in enumerate(x):
        if path[idx].numpy().decode() != "./tinyml_contest_data_training/S48-VT-145.txt":
            continue
        ...
        for i in range(len(thresholds)):
            ...
            max_peak = max(max_peak, numPeaks, numUnpeaks)
            min_peak = min(min_peak, numPeaks, numUnpeaks)
...
'''
interpreter = tf.lite.Interpreter(model_path="./validate/f16/FLOAT16.tflite")
interpreter.allocate_tensors()
print(get_metric("train", interpreter))
'''

subs = get_subjects("./data_indices/train_indice.csv")
for s in subs:
    print(f"\"{s}\", ", end="")
exit(0)

weight_dir = "./validate/f16"
print(get_metric_by_weight("test", np.load(f"{weight_dir}/dense1_w.npy"),
                           np.load(f"{weight_dir}/dense1_b.npy"),
                           np.load(f"{weight_dir}/dense2_w.npy"),
                           np.load(f"{weight_dir}/dense2_b.npy"),
                           np.load(f"{weight_dir}/dense3_w.npy"),
                           np.load(f"{weight_dir}/dense3_b.npy")))
exit(0)

parser = argparse.ArgumentParser()
# parser.add_argument("--tflite", help="path of tflite file", required=True)
# parser.add_argument("--multiclass", help="whether it is a multi-class model", default=False)
args = parser.parse_args()

print(f"load {args.tflite}")

def analyse(quant_type):
    test_scores = []
    train_scores = []
    for i in range(1):
        path = f"{args.tflite}/{i}/"
        path = path + [file for file in os.listdir(path) if quant_type in file][0]
        interpreter = tf.lite.Interpreter(model_path=path)
        print(f"load {path}")
        interpreter.allocate_tensors()
        fb, g, det = get_metric("test", interpreter, args.multiclass)
        test_scores.append(det)
        fb, g, det = get_metric("train", interpreter, args.multiclass)
        train_scores.append(det)

    combines = [train+test for train, test in zip(train_scores, test_scores)]
    max_idx = np.argmax(combines)
    result = (np.mean(test_scores),np.std(test_scores), 
              np.mean(train_scores), np.std(train_scores),
              f"{test_scores[max_idx]}+{train_scores[max_idx]}={combines[max_idx]}",
              max_idx)
...
# quant_types = ["INT8", "FLOAT16", "NONE"]

'''
import multiprocessing
quant_types = ["FLOAT16", "None"]
pool = multiprocessing.Pool(processes=len(quant_types))
results = pool.map(analyse, quant_types)
pool.close()
pool.join()
result_path = "./train_result/cluster.csv"
with open(result_path, "w+") as file:
    file.write("Quant Type,Avg Test,Std Test,Avg Train,Std Train,Max Test+Train,Max ID\n")
    for (quant_type, r) in zip(quant_types, results):
        result_str = ",".join(r)
        file.write(f"{quant_type},{result_str}\n")
print(f"Done, result write to {result_path}")
'''